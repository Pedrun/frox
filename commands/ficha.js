    ...
    data: new SlashCommandBuilder()
            ...
            group
                ...
                .addSubcommand((subCommand) =>
                    subCommand
                        .setName('adicionar')
                        .setDescription(
                            'Adiociona um novo atributo para ficha atual'
                        )
                        .addStringOption((option) =>
                            option
                                .setName('atributos')
                                .setDescription(
                                    'Sintaxe: "Força Armadura Defesa" ou "Força:2 Armadura Defesa:0"'
                                )
                                .setRequired(true)
                        )
                )
            ...
            group
                ...
                .addSubcommand((subCommand) =>
                    subCommand
                        .setName('adicionar')
                        .setDescription(
                            'Adiociona uma barra para a ficha atual'
                        )
                        .addStringOption((option) =>
                            option
                                .setName('nome')
                        ...
                        .addStringOption((option) =>
                            option
                                .setName('máximo')
                                .setDescription(
                                    'O atributo que servirá de limite da barra'
                                )
                                .setRequired(true)
                        )
                )
                    ...
                    subCommand
                        ...
                        .addStringOption((option) =>
                            option
                                .setName('barra')
                                .setDescription(
                                    'O nome da barra que será removida'
                                )
                                .setRequired(true)
                        )
                )
        ...
        .addSubcommand((subCommand) =>
            subCommand
                .setName('cor')
                .setDescription(
                    'Define a cor da ficha (deixe em branco para copiar a cor do seu cargo)'
                )
                .addStringOption((option) =>
                    option
                        .setName('cor')
                        .setDescription(
                            'A nova cor da ficha (deixe em branco para copiar a cor do seu cargo)'
                        )
                )
        )
        .addSubcommand((subCommand) =>
    ...
    async execute(interaction, client) {
        ...
        if (!player.card)
            return interaction.reply({
                content: `${interaction.user}, **Você não tem uma ficha.** *Crie uma lista usando \` /fichas criar \`*`,
                ephemeral: true,
            });
        ...
        if (group === 'atributo' && subCommand === 'adicionar') {
            ...
            if (attributes.length + player.card.attributes.size > 64) {
                return interaction.reply({
                    content: `${interaction.user}, O máximo de atributos por ficha é 64`,
                    ephemeral: true,
                });
            }
            let addedAttributes = [];
            ...
            interaction.reply({
                content: `Os atributos **"${addedAttributes.join(
                    '**, **'
                )}"** foram adcionados com sucesso`,
            });
        ...
        if (group === 'atributo' && subCommand === 'remover') {
            ...
            if (!player.card.hasAttr(atributo))
                return interaction.reply({
                    content: `${interaction.user} **Você não tem nenhum atributo chamado "${atributo}"**`,
                    ephemeral: true,
                });

            player.card.removeAttr(atributo);
            interaction.reply({
                content: `O atributo **"${atributo}"** foi removido de sua lista`,
            });
            client.saveInstances();
            return;
        }
        ...
        if (group === 'atributo' && subCommand === 'limpar') {
            ...
            interaction.reply({
                content: `Todos atributos de **${player.card.name}** foram excluídos`,
            });
            client.saveInstances();
            return;
        }
        ...
        if (group === 'barra' && subCommand === 'adicionar') {
            if (player.card.bars.length >= 6)
                return interaction.reply({
                    content: `Cada ficha só pode ter até 6 barras`,
                    ephemeral: true,
                });
            ...
            player.card.bars.push(
                new CardBar({ name: nome, value: valor, max: máximo })
            );
            interaction.reply({
                content: `A barra **${nome}** foi adicionada com sucesso`,
            });
            client.saveInstances();
            return;
        }
        ...
        if (group === 'barra' && subCommand === 'remover') {
            ...
            if (index < 0)
                return interaction.reply({
                    content: `${interaction.user}, Não encontrei nenhuma barra com o nome de **"${barra}"**`,
                    ephemeral: true,
                });

            player.card.bars.splice(index, 1);
            interaction.reply({
                content: `Removido barra **"${barra}"** com sucesso`,
            });
            client.saveInstances();
            return;
        }
        ...
        if (subCommand === 'cor') {
            ...
            if (!colorRegex.test(cor))
                return interaction.reply({
                    content: `${interaction.user}, **"${cor}" não é um código HEX de cor válido**`,
                    ephemeral: true,
                });
        ...
        if (subCommand === 'privar') {
            ...
            interaction.reply({
                content: `Agora sua ficha é **${
                    player.card.isPrivate ? 'Privada' : 'Pública'
                }**`,
            });
            client.saveInstances();
            return;
